...
def service_load_exercise_templates(request):
    return compile_gwt_response(
            serializers.serialize('json',
                    ExerciseTemplate.objects.order_by('position').filter(workout_template=WorkoutTemplate.objects.get(id=workout_template_id)),
                    relations = ()
            )
    )
